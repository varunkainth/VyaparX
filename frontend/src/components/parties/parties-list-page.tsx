...
export function PartiesListPage({ filterType: initialFilterType = "all" }: PartiesListPageProps) {
  ...
  const [filterType, setFilterType] = useState<"all" | "customer" | "supplier" | "both">(initialFilterType)
        ...
        <div className="flex flex-1 flex-col gap-4 md:gap-6 p-4 md:p-6 pb-20 md:pb-6">
          ...
          <div className="flex flex-col gap-3 sm:gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <h1 className="text-xl sm:text-2xl md:text-3xl font-bold tracking-tight">{getPageTitle()}</h1>
                <p className="text-xs sm:text-sm text-muted-foreground mt-0.5 sm:mt-1 truncate">
                  {getPageDescription()}
                </p>
              </div>
            </div>
            <Button onClick={() => router.push("/parties/create")} className="cursor-pointer w-full sm:hidden">
          ...
          <div className="grid gap-3 md:gap-4 grid-cols-2 lg:grid-cols-4">
            <Card>
              ...
              <CardContent className="p-3 pt-0 md:p-6 md:pt-0">
                <div className="text-lg sm:text-xl md:text-2xl font-bold">{parties.length}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 p-3 md:p-6 md:pb-2">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 p-3 md:p-6 md:pb-2">
                <CardTitle className="text-xs md:text-sm font-medium">Suppliers</CardTitle>
          ...
          <Card>
            <CardHeader className="p-3 md:p-6">
              <CardTitle className="text-base md:text-lg">All Parties ({filteredParties.length})</CardTitle>
              <CardDescription className="text-xs md:text-sm">
                <span className="hidden sm:inline">Click on a party to view details and edit</span>
                    ...
                    {filteredParties.map((party) => (
                      ...
                      >
                        <div className="flex items-start sm:items-center gap-4 flex-1 w-full">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <Badge variant={getPartyTypeColor(party.party_type)} className="capitalize text-xs">
                                {party.party_type}
                              </Badge>
                              {!party.is_active && (
                        ...
                        <div className="text-left sm:text-right w-full sm:w-auto">
                          ...
                          <p className={`font-semibold ${party.current_balance > 0 ? "text-green-600" : party.current_balance < 0 ? "text-red-600" : ""}`}>
                    ...
                    {filteredParties.map((party) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-2">
                          <p className="font-medium text-sm truncate flex-1">{party.name}</p>
                        ...
                        <MobileTableCell label="Balance">
                          <span className={`font-semibold text-xs ${party.current_balance > 0 ? "text-green-600" : party.current_balance < 0 ? "text-red-600" : ""}`}>
                            {formatBalance(party.current_balance)}
                          </span>
                        </MobileTableCell>
                        {!party.is_active && (
