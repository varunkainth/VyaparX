...
import { Users, Info, UserPlus, RefreshCw, MailPlus, Link as LinkIcon, Ban, Copy } from "lucide-react"
...
const inviteStatusMeta: Record<InviteStatus, { badgeVariant: "default" | "secondary" | "outline"; label: string }> = {
...
export function TeamMembersPage() {
    ...
    const handleResendInvite = async (invite: ListedBusinessInvite) => {
        ...
        try {
            ...
            if (typeof navigator !== "undefined" && navigator.clipboard?.writeText) {
                ...
                toast.success(`Email service unavailable. Copied fresh invite link for ${invite.email}.`)
                return
            }

            toast.success("Invite refreshed. Copy the link from the API response if needed.")
        } catch (error) {
    ...
    const handleRevokeInvite = async (inviteId: string) => {
        ...
        try {
            const revokedInvite = await businessService.revokeInvite(currentBusiness.id, inviteId)
            setInvites((current) =>
                current.map((invite) => (invite.id === inviteId ? revokedInvite : invite))
            )
        ...
        return (
            ...
            >
                <div className="min-w-0 space-y-1">
                    <p className="truncate font-medium text-sm md:text-base">{invite.email}</p>
                    <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground md:text-sm">
                        ...
                        {isPending ? (
                            ...
                            <span>
                                {statusMeta.label} {new Date(activityTimestamp).toLocaleString()}
                            </span>
                        ) : null}
                    </div>
                </div>
    ...
    const filteredInvites = invites
        .filter((invite) => inviteFilter === "all" ? true : invite.status === inviteFilter)
        .filter((invite) => invite.email.toLowerCase().includes(inviteSearch.trim().toLowerCase()))
        .sort((left, right) => {
                    ...
                    <div className="flex flex-1 flex-col gap-4 md:gap-6 p-4 md:p-6 pb-20 md:pb-6">
                        <div className="flex flex-col gap-3 md:gap-0 md:flex-row md:items-center md:justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                >
                                    <RefreshCw className={`h-4 w-4 mr-2 ${loadingMembers || loadingInvites ? 'animate-spin' : ''}`} />
                                    Refresh
                                </Button>
                                {canManageMembers && (
                            ...
                            <Card>
                                <CardHeader className="gap-4">
                                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                        <div>
                                            <CardTitle className="text-base md:text-lg">Invite Tracker</CardTitle>
                                            <CardDescription className="mt-1 text-xs md:text-sm">
                                                Review outstanding invites, resend them, revoke access, or inspect invite history.
                                            </CardDescription>
                                        </div>
                                        <div className="flex flex-wrap gap-2">
                                            {inviteFilterOptions.map((option) => (
                                                <Button
                                                    key={option.value}
                                                    type="button"
                                                    variant={inviteFilter === option.value ? "default" : "outline"}
                                                    size="sm"
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {inviteSortOptions.map((option) => (
                                                <Button
                                                    key={option.value}
                                                    type="button"
                                                    variant={inviteSort === option.value ? "default" : "outline"}
                                                    size="sm"
                                    ...
                                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                                        {(["pending", "accepted", "revoked", "expired"] as InviteStatus[]).map((status) => (
                                            <div key={status} className="rounded-lg border p-4">
                                                <div className="flex items-center justify-between gap-2">
                                                    <Badge variant={inviteStatusMeta[status].badgeVariant}>
