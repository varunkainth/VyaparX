...
export function CreateInvoicePage({ invoiceType }: CreateInvoicePageProps) {
  ...
  const [sourceInvoiceNumber, setSourceInvoiceNumber] = useState<string | null>(
    null,
  );
    ...
    try {
      ...
      const filtered =
        invoiceType === "sales"
          ...
          : data.filter(
              (p) => p.party_type === "supplier" || p.party_type === "both",
            );
      setParties(filtered);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const loadSourceInvoice = async () => {
      ...
      try {
        const sourceInvoice = await invoiceService.getInvoice(
          currentBusiness.id,
          sourceInvoiceId,
        );
        ...
        if (sourceInvoice.invoice_type !== invoiceType) {
          toast.error(
            `This ${sourceInvoice.invoice_type} invoice cannot be loaded in the ${invoiceType} invoice form.`,
          );
  ...
  const getInventoryItemUnitPrice = useCallback(
    (item: InventoryItem) => {
      return round2(
        invoiceType === "sales" ? item.selling_price : item.purchase_price,
      );
    },
    [invoiceType, round2],
  );

  const applyInventoryItemToForm = useCallback(
  ...
  const handleAddToMasterInventory = async (index: number) => {
    ...
    if (!item.item_name || !item.unit_price) {
      toast.error(
        "Please fill in item name and unit price before adding to inventory",
      );
      return;
    }
  ...
  const validateStockForSales = (): { valid: boolean; message?: string } => {
    ...
    for (const item of watchItems) {
      ...
      if (inventoryItem.current_stock <= 0) {
        stockErrors.push(
          `${item.item_name}: Out of stock (available: ${inventoryItem.current_stock})`,
        );
  ...
  const onSubmit = async (data: CreateInvoiceFormData) => {
    ...
    try {
      ...
      const subtotal = round2(
        itemsWithCalculations.reduce((sum, item) => {
      ...
      const totalTax = round2(
        itemsWithCalculations.reduce(
          (sum, item) =>
            sum + item.cgst_amount + item.sgst_amount + item.igst_amount,
          0,
        ),
      );
      ...
      const invoiceData = {
        ...data,
        ...(isRevisionMode && sourceInvoiceId
          ...
          <div className="flex min-h-screen items-center justify-center p-4">
            <Card className="w-full max-w-lg">
              <CardHeader>
                ...
                <CardDescription>
                  Your role can view invoices but cannot create or revise
                  invoices.
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Review the details, update anything needed, and save as
                        a new invoice.
                      </p>
                    </div>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <Field className="sm:col-span-2">
                        ...
                        >
                          <SelectTrigger>
                            <SelectValue
                              placeholder={`Select ${invoiceType === "sales" ? "customer" : "supplier"}`}
                            />
                          </SelectTrigger>
                          <SelectContent>
                            {parties.map((party) => (
                      ...
                      <Field>
                        ...
                        <Select
                          ...
                          onValueChange={(value) =>
                            setValue("place_of_supply", value)
                          }
                        >
                          <SelectTrigger>
                    ...
                    {watchPlaceOfSupply && currentBusiness.state_code && (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-6">
                                ...
                                currentBusiness.state_code ? (
                                  <>
                                    Within{" "}
                                    {
                                      INDIAN_STATES.find(
                                        (s) =>
                                          s.code === currentBusiness.state_code,
                                      )?.name
                                    }
                                  </>
                                ) : (
                          ...
                          <div
                            className={`flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg ${
                              watchPlaceOfSupply === currentBusiness.state_code
                            ...
                            inventoryItems.map((item) => {
                              ...
                              return (
                                <SelectItem
                                  ...
                                  value={item.id}
                                  disabled={
                                    invoiceType === "sales" && isOutOfStock
                                  }
                                  className={
                                    invoiceType === "sales" && isOutOfStock
                                      ? "opacity-50"
                          ...
                          <div className="space-y-3 md:space-y-4">
                                ...
                                {invoiceType === "purchase" &&
                                  !watchItems[index]?.item_id && (
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() =>
                                        handleAddToMasterInventory(index)
                                      }
                            ...
                            <div className="grid gap-3 md:gap-4 sm:grid-cols-2">
                              <Field className="sm:col-span-2">
                                ...
                                <Select
                                  value={
                                    watchItems[index]?.item_id || undefined
                                  }
                                  onValueChange={(value) =>
                                ...
                                {watchItems[index]?.item_id &&
                                  (() => {
                                    const selectedItem = inventoryItems.find(
                                      (i) => i.id === watchItems[index].item_id,
                                    );
                                    if (selectedItem) {
                                      const currentQty =
                              ...
                              <Field>
                                ...
                                <Select
                                  value={
                                    watchItems[index]?.gst_rate?.toString() ||
                                    "18"
                                  }
                                  onValueChange={(value) =>
                                    setValue(
                ...
                <CardContent>
                  <div className="space-y-2 md:space-y-3">
                    ...
                    <div className="flex justify-between text-sm md:text-base">
                      ...
                      <span className="font-medium">
                        {formatCurrency(totals.totalBeforeRounding)}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm md:text-base">
