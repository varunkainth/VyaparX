...
export function PaymentsListPage() {
  ...
  useEffect(() => {
    filterPayments()
  }, [payments, searchQuery, paymentTypeFilter, paymentModeFilter, showReconciled, showUnreconciled])
        ...
        <div className="flex flex-1 flex-col gap-4 md:gap-6 p-4 md:p-6 pb-20 md:pb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs sm:text-sm text-muted-foreground mt-0.5 sm:mt-1 truncate">
                  <span className="hidden sm:inline">Track all payment transactions</span>
                  <span className="sm:hidden">Track transactions</span>
                </p>
              </div>
            </div>
            <Button onClick={() => router.push("/payments/record")} className="cursor-pointer w-full sm:hidden">
          ...
          <div className="grid gap-3 md:gap-4 grid-cols-2 lg:grid-cols-4">
            <Card>
              ...
              <CardContent className="p-3 pt-0 md:p-6 md:pt-0">
                <div className="text-lg sm:text-xl md:text-2xl font-bold">{payments.length}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 p-3 md:p-6 md:pb-2">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 p-3 md:p-6 md:pb-2">
                <CardTitle className="text-xs md:text-sm font-medium">Made</CardTitle>
                ...
                <div className="grid gap-3 md:gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div className="flex flex-col gap-2 sm:col-span-2 lg:col-span-1">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <label htmlFor="reconciled" className="text-xs md:text-sm cursor-pointer">
                        Reconciled
                      </label>
                    </div>
                    <div className="flex items-center gap-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setShowUnreconciled(checked as boolean)}
                      />
                      <label htmlFor="unreconciled" className="text-xs md:text-sm cursor-pointer">
                        Unreconciled
                      </label>
                    </div>
                  </div>
          ...
          <Card>
            <CardHeader className="p-3 md:p-6">
              <CardTitle className="text-base md:text-lg">All Payments ({filteredPayments.length})</CardTitle>
              <CardDescription className="text-xs md:text-sm">
                <span className="hidden sm:inline">Click on a payment to view details</span>
              ...
              {isLoading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm md:text-base text-muted-foreground">
                    {searchQuery || paymentTypeFilter !== "all" || paymentModeFilter !== "all"
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {payment.is_reconciled ? (
                                ...
                                <Badge variant="outline" className="text-orange-600 border-orange-600 text-xs">
                                  ...
                                  {payment.payment_mode === "cash" ? "Pending" : "Unreconciled"}
                                </Badge>
                              )}
                            </div>
                            <div className="flex items-center gap-4 mt-1 text-sm text-muted-foreground">
                              <span>{formatDate(payment.payment_date)}</span>
                              <span>{getPaymentModeLabel(payment.payment_mode)}</span>
                              {payment.upi_ref && <span>UPI: {payment.upi_ref}</span>}
                    ...
                    {filteredPayments.map((payment) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="font-medium text-sm truncate flex-1">{payment.party_name}</p>
                          <Badge variant={payment.payment_type === "received" ? "default" : "secondary"} className="text-xs flex-shrink-0">
                            {payment.payment_type === "received" ? "Received" : "Made"}
                          </Badge>
                        </div>
                        ...
                        <MobileTableCell label="Mode">
                          <span className="text-xs">{getPaymentModeLabel(payment.payment_mode)}</span>
                        </MobileTableCell>
