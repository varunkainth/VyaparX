...
export function RecordPaymentPage() {
  ...
  const [isLoadingAvailableInvoices, setIsLoadingAvailableInvoices] =
    useState(false);
  ...
  useEffect(() => {
    devLog("Party/PaymentType changed:", {
      watchPartyId,
      paymentType,
  ...
  const fetchInvoice = async () => {
    ...
    try {
      ...
      setValue(
        "payment_type",
        data.invoice_type === "sales" ? "received" : "made",
      );
  ...
  const onSubmit = async (data: RecordPaymentFormData) => {
    ...
    if (!targetInvoiceId) {
      toast.error(
        "Please select an invoice to allocate this payment. Create a new invoice first if needed.",
      );
      return;
    }
          ...
          <div className="flex min-h-screen items-center justify-center p-4">
            <Card className="w-full max-w-lg">
              <CardHeader>
                ...
                <CardDescription>
                  Your role can view payments but cannot record new ones.
                </CardDescription>
              </CardHeader>
              <CardContent>
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col gap-6 p-6">
          ...
          {!invoice && !invoiceId && (
            <Alert>
              <AlertDescription>
                Select a party and invoice to record payment. The payment will
                be allocated to the selected invoice.
              </AlertDescription>
            </Alert>
          )}
                  ...
                  <FieldGroup>
                    <Field>
                      ...
                      <Select
                        value={paymentType}
                        onValueChange={(value) =>
                          setValue("payment_type", value as "received" | "made")
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                      ...
                      availableInvoices.length === 0 && (
                        <Alert className="border-orange-200 bg-orange-50 dark:bg-orange-950/20 dark:border-orange-800">
                          <AlertDescription className="text-sm">
                            ...
                            - All invoices for this party are fully paid. Please
                            create a new invoice first before recording payment.
                          </AlertDescription>
                        </Alert>
                      )}
                              ...
                              <div className="mt-2 p-3 rounded-lg bg-muted/50 border text-sm space-y-1">
                                <div className="flex justify-between">
                                  ...
                                  <span className="font-medium">
                                    ₹{selectedInv.grand_total.toFixed(2)}
                                  </span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="text-muted-foreground">
                                  ...
                                  <span className="font-medium text-green-600">
                                    ₹{selectedInv.amount_paid.toFixed(2)}
                                  </span>
                                </div>
                                <Separator className="my-1" />
                                <div className="flex justify-between">
                                  ...
                                  <span className="font-bold text-primary">
                                    ₹{selectedInv.balance_due.toFixed(2)}
                                  </span>
                                </div>
                              </div>
                            ) : null;
                  ...
                  <FieldGroup>
                    <Field>
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          setValue(
                            "payment_mode",
                            value as RecordPaymentFormData["payment_mode"],
                          )
                        }
                      >
                        <SelectTrigger>
                    ...
                    <Alert className="border-green-200 bg-green-50 dark:bg-green-950/20 dark:border-green-800">
                      <AlertDescription className="text-xs">
                        <span className="font-semibold">Auto-reconciled:</span>{" "}
                        Payments are marked reconciled when recorded. Add UPI,
                        cheque, or bank references when available for cleaner
                        audit history.
                      </AlertDescription>
                    </Alert>
                  </FieldGroup>
