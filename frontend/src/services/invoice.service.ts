  ...
  return {
    ...
    subtotal:
      typeof invoice.subtotal === "string"
        ...
        : invoice.subtotal,
    total_discount:
      typeof invoice.total_discount === "string"
        ? parseFloat(invoice.total_discount)
        : invoice.total_discount,
    taxable_amount:
      typeof invoice.taxable_amount === "string"
        ? parseFloat(invoice.taxable_amount)
        : invoice.taxable_amount,
    cgst_amount:
      typeof invoice.cgst_amount === "string"
        ? parseFloat(invoice.cgst_amount)
        : invoice.cgst_amount,
    sgst_amount:
      typeof invoice.sgst_amount === "string"
        ? parseFloat(invoice.sgst_amount)
        : invoice.sgst_amount,
    igst_amount:
      typeof invoice.igst_amount === "string"
        ? parseFloat(invoice.igst_amount)
        : invoice.igst_amount,
    total_tax:
      typeof invoice.total_tax === "string"
        ? parseFloat(invoice.total_tax)
        : invoice.total_tax,
    round_off:
      ...
      invoice.items?.map((item: any) => ({
        ...
        unit_price:
          typeof item.unit_price === "string"
            ...
            : item.unit_price,
        discount_pct:
          typeof item.discount_pct === "string"
            ? parseFloat(item.discount_pct)
            : item.discount_pct,
        discount_amount:
          typeof item.discount_amount === "string"
            ? parseFloat(item.discount_amount)
            : item.discount_amount,
        taxable_value:
          typeof item.taxable_value === "string"
            ? parseFloat(item.taxable_value)
            : item.taxable_value,
        gst_rate:
          typeof item.gst_rate === "string"
            ? parseFloat(item.gst_rate)
            : item.gst_rate,
        cgst_rate:
          typeof item.cgst_rate === "string"
            ? parseFloat(item.cgst_rate)
            : item.cgst_rate,
        sgst_rate:
          typeof item.sgst_rate === "string"
            ? parseFloat(item.sgst_rate)
            : item.sgst_rate,
        igst_rate:
          typeof item.igst_rate === "string"
            ? parseFloat(item.igst_rate)
            : item.igst_rate,
        cgst_amount:
          typeof item.cgst_amount === "string"
            ? parseFloat(item.cgst_amount)
            : item.cgst_amount,
        sgst_amount:
          typeof item.sgst_amount === "string"
            ? parseFloat(item.sgst_amount)
            : item.sgst_amount,
        igst_amount:
          typeof item.igst_amount === "string"
...
export const invoiceService = {
  async listInvoices(
    ...
    if (query?.include_cancelled !== undefined)
      params.append("include_cancelled", String(query.include_cancelled));
    if (query?.page) params.append("page", String(query.page));
    if (query?.limit) params.append("limit", String(query.limit));

    const url = `/api/v1/businesses/${businessId}/invoices${params.toString() ? `?${params.toString()}` : ""}`;
    const response =
      await apiClient.get<ApiResponse<PaginatedResponse<any>>>(url);
  ...
  async testEmailConfig(
    testEmail?: string,
  ): Promise<{ recipient: string; sent_at: string }> {
    const response = await apiClient.post<
      ApiResponse<{ recipient: string; sent_at: string }>
    >(`/api/v1/email/test`, testEmail ? { test_email: testEmail } : {});
    return response.data.data;
  },
