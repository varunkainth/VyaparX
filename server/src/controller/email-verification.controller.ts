...
export async function resendVerificationEmailHandler(
    ...
    if (!isDevelopment) {
        const latestToken = await emailVerificationRepository.getLatestToken(userId);
        if (latestToken) {
            const timeSinceLastToken = Date.now() - new Date(latestToken.created_at).getTime();
            const twoMinutes = 2 * 60 * 1000;
            
            if (timeSinceLastToken < twoMinutes) {
                const secondsRemaining = Math.ceil((twoMinutes - timeSinceLastToken) / 1000);
                throw new AppError(
