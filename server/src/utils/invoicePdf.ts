...
const templateSchema = z.object({
    layout: z.enum(["classic", "modern", "compact", "bill_pro", "bill_pro_legacy"]),
...
const templatesConfigSchema = z.object({
    defaultTemplate: z.enum(["classic", "modern", "compact", "bill_pro", "bill_pro_legacy"]),
    templates: z.object({
...
const loadTemplatesConfig = (): TemplatesConfig => {
    const filePath = fileURLToPath(new URL("../config/invoice-templates.json", import.meta.url));
    const raw = readFileSync(filePath, "utf-8");
    const parsed = templatesConfigSchema.safeParse(JSON.parse(raw));
    if (!parsed.success) {
        throw new Error(`Invalid invoice templates config: ${parsed.error.message}`);
    }
    return parsed.data;
};
...
const renderItems = (doc: PDFKit.PDFDocument, data: InvoicePdfData, config: TemplateConfig) => {
...
// ════════════════════════════════════════════════════════════════════════════
const renderClassic = (doc: PDFKit.PDFDocument, data: InvoicePdfData, config: TemplateConfig) => {
    const x = 50;
    ...
    doc.font("Helvetica-Bold").fontSize(24).text(getText(data.businessName).toUpperCase(), x, y);
    doc.fillColor("#555555").fontSize(10).text(getText(data.business?.address_line1), x, y + 28);
    doc.text(`${getText(data.business?.city)}, ${getText(data.business?.state)}`, x, y + 40);

    doc.fillColor(config.headerBackgroundColor || "#000000").font("Helvetica-Bold").fontSize(28)
    ...
    // Divider
    doc.moveTo(x, y).lineTo(rightX, y).lineWidth(1).strokeColor("#DDDDDD").stroke();
    y += 15;

    // Meta: Bill To & Invoice Info
    doc.font("Helvetica-Bold").fontSize(10).text("BILL TO:", x, y);
    doc.font("Helvetica").fontSize(10).text(getText(data.partyName).toUpperCase(), x, y + 15);
    ...
    doc.font("Helvetica").text(getText(data.invoice.invoice_number), infoX + 90, y);

    doc.font("Helvetica-Bold").text("Invoice Date:", infoX, y + 15);
    doc.font("Helvetica").text(formatDate(data.invoice.invoice_date), infoX + 90, y + 15);

    doc.font("Helvetica-Bold").text("Status:", infoX, y + 30);
    doc.font("Helvetica").text(getText(data.invoice.payment_status).toUpperCase(), infoX + 90, y + 30);

    y += 70;

    // Table Header
    doc.moveTo(x, y).lineTo(rightX, y).lineWidth(2).strokeColor("#000000").stroke();
    y += 8;
    ...
    doc.text("Amount", colX[3]!, y, { width: colX[4]! - colX[3]!, align: "right" });

    y += 15;
    doc.moveTo(x, y).lineTo(rightX, y).lineWidth(1).strokeColor("#EEEEEE").stroke();
    y += 10;
...
// ════════════════════════════════════════════════════════════════════════════
const renderModern = (doc: PDFKit.PDFDocument, data: InvoicePdfData, config: TemplateConfig) => {
    ...
    doc.fillColor(txtColor).font("Helvetica-Bold").fontSize(28).text(config.title.toUpperCase(), 40, 40);
    ...
    doc.font("Helvetica-Bold").fontSize(14).text(getText(data.businessName).toUpperCase(), 40, 40, { width: doc.page.width - 80, align: "right" });
    doc.font("Helvetica").fontSize(9).text(getText(data.business?.address_line1), 40, 60, { width: doc.page.width - 80, align: "right" });
    doc.text(`GSTIN: ${getText(data.business?.gstin)}`, 40, 75, { width: doc.page.width - 80, align: "right" });
    ...
    // Bill To
    doc.fillColor("#666666").font("Helvetica-Bold").fontSize(9).text("INVOICE TO", x, y);
    doc.fillColor("#000000").font("Helvetica-Bold").fontSize(12).text(getText(data.partyName).toUpperCase(), x, y + 15);
    doc.font("Helvetica").fontSize(9).fillColor("#555555")
        .text(getText(data.party?.address), x, y + 30)
        .text(`${getText(data.party?.city)}, ${getText(data.party?.state)}`, x, y + 42)
    ...
    doc.text("Quantity", colX[1]!, headerY, { width: colX[2]! - colX[1]!, align: "center" });
    doc.text("Price", colX[2]!, headerY, { width: colX[3]! - colX[2]!, align: "right" });
    doc.text("Amount", colX[3]!, headerY, { width: colX[4]! - colX[3]!, align: "right" });
    ...
    data.invoice.items.forEach((item, index) => {
        ...
        // Zebra striping
        if (index % 2 === 1) doc.rect(x, y - 5, rightX - x, itemH + 10).fill("#F9FAFB");
...
// ════════════════════════════════════════════════════════════════════════════
const renderCompact = (doc: PDFKit.PDFDocument, data: InvoicePdfData, config: TemplateConfig) => {
    ...
    const drawDashedLine = (yPos: number) => {
        doc.moveTo(x, yPos).lineTo(x + width, yPos).lineWidth(1).strokeColor("#AAAAAA").dash(3, { space: 3 }).stroke();
        doc.undash(); // Reset dash so it doesn't break other lines
    };

    // Header
    doc.font("Helvetica-Bold").fontSize(16).text(getText(data.businessName).toUpperCase(), x, y, { width, align: "center" });
    y += 20;
    doc.font("Helvetica").fontSize(8).fillColor("#555555")
        .text(getText(data.business?.address_line1), x, y, { width, align: "center" })
        .text(getText(data.business?.city), x, y + 10, { width, align: "center" })
        .text(`GSTIN: ${getText(data.business?.gstin)}`, x, y + 20, { width, align: "center" });
    ...
    // Invoice Meta
    doc.fillColor("#000000").font("Helvetica-Bold").fontSize(12).text(config.title.toUpperCase(), x, y, { width, align: "center" });
    y += 20;
    doc.font("Helvetica").fontSize(9);
    doc.text(`No: ${data.invoice.invoice_number}`, x, y);
    doc.text(formatDate(data.invoice.invoice_date), x, y, { width, align: "right" });
    ...
    data.invoice.items.forEach(item => {
        ...
        doc.text(rs(item.taxable_value ?? item.total_amount), x, y, { width, align: "right" });
...
// ════════════════════════════════════════════════════════════════════════════
const renderBillPro = (doc: PDFKit.PDFDocument, data: InvoicePdfData, config: TemplateConfig) => {
    const x = 40;
    ...
    doc.text(getText(data.businessName).toUpperCase(), x, currentY + 6, { width, align: "center" });
    currentY += 22;
    ...
    const cityParts = [getText(data.business?.city, ""), getText(data.business?.state, "")]
    ...
    // Bold Business GSTIN
    doc.font("Helvetica-Bold").text(`GSTIN No ${getText(data.business?.gstin)}`, x, currentY, { width, align: "center" });
    currentY += 14;
    doc.moveTo(x, currentY).lineTo(totalX, currentY).stroke("#000000");
    ...
    doc.moveTo(mCol1X, metaY).lineTo(mCol1X, metaY + metaH).stroke("#000000");
    doc.moveTo(mCol2X, metaY).lineTo(mCol2X, metaY + metaH).stroke("#000000");
    doc.moveTo(mCol3X, metaY).lineTo(mCol3X, metaY + metaH).stroke("#000000");

    // Horizontal split for Invoice No & Dated
    doc.moveTo(mCol2X, metaY + 28).lineTo(totalX, metaY + 28).stroke("#000000");

    // --- Bill To ---
    doc.font("Helvetica-Bold").fontSize(7).text("Bill to", x + 4, metaY + 4);
    doc.fontSize(9).text(getText(data.partyName).toUpperCase(), x + 4, metaY + 16, { width: mCol1W - 8 });
    doc.font("Helvetica").fontSize(8);
    ...
    if (pCity.length > 0) {
        const pLine = (pPin.length > 0 && pPin !== "-") ? `${pCity} - ${pPin}` : pCity;
        doc.text(pLine, x + 4, bY, { width: mCol1W - 8 });
        bY += 10;
    }

    // Bold Party GSTIN
    doc.font("Helvetica-Bold").text(`GSTIN NO. ${getText(data.party?.gstin)}`, x + 4, bY, { width: mCol1W - 8 });

    // --- Place of Supply (Updated to show Name and Address) ---
    doc.font("Helvetica-Bold").fontSize(7).text("Place of Supply", mCol1X + 4, metaY + 4);
    doc.fontSize(9).text(getText(data.partyName).toUpperCase(), mCol1X + 4, metaY + 16, { width: mCol2W - 8 });
    ...
    if (pCity.length > 0) {
        const pLine = (pPin.length > 0 && pPin !== "-") ? `${pCity} - ${pPin}` : pCity;
        doc.text(pLine, mCol1X + 4, posAddrY, { width: mCol2W - 8 });
        posAddrY += 10;
    }
    ...
    // --- Invoice No ---
    doc.font("Helvetica").fontSize(7).text("INVOICE No", mCol2X + 4, metaY + 4, { align: "center", width: mCol3W - 8 });
    doc.font("Helvetica-Bold").fontSize(9).text(getText(data.invoice.invoice_number), mCol2X + 4, metaY + 45, { align: "center", width: mCol3W - 8 });

    // --- Dated ---
    doc.font("Helvetica").fontSize(7).text("Dated", mCol3X + 4, metaY + 4, { align: "center", width: mCol4W - 8 });
    doc.font("Helvetica-Bold").fontSize(9).text(formatDate(data.invoice.invoice_date), mCol3X + 4, metaY + 45, { align: "center", width: mCol4W - 8 });
    ...
    const hdrH = 20;
    doc.font("Helvetica-Bold").fontSize(8);
    const headers = ["Description of Goods", "HSN CODE", "QTY", "Units", "RATE", "Amount"];
    headers.forEach((h, i) => {
    ...
    data.invoice.items.forEach((item) => {
        ...
        doc.font("Helvetica").fontSize(9);
        doc.text(getText(item.hsn_code), tColX[1]! + 4, iy, { width: tColW[1]! - 8, align: "center" });
    ...
    // ════════════════════════════════════
    const cgstTotal = data.invoice.items.reduce((s, r) => s + asNumber(r.cgst_amount), 0);
    const sgstTotal = data.invoice.items.reduce((s, r) => s + asNumber(r.sgst_amount), 0);
    const igstTotal = data.invoice.items.reduce((s, r) => s + asNumber(r.igst_amount), 0);
    ...
    const taxes: [string, number][] = [["Taxable Value", asNumber(data.invoice.taxable_amount)]];
    ...
    const finalTableBottomY = Math.max(currentY + minTableHeight, iy + taxSectionHeight + 30);
    ...
    taxes.forEach(([label, amount]) => {
        doc.font(label.startsWith("ADD") ? "Helvetica-Bold" : "Helvetica").fontSize(9);
        doc.text(label, tColX[0]! + 4, taxY, { width: tColW[0]! - 8 });
        doc.text(rs(amount), tColX[5]! + 4, taxY, { width: tColW[5]! - 14, align: "right" });
        taxY += 16;
    });

    // Draw inner vertical columns strictly inside the Items table bounds
    for (let i = 1; i < tColX.length - 1; i++) {
        doc.moveTo(tColX[i]!, tableY).lineTo(tColX[i]!, finalTableBottomY).stroke("#000000");
    }
    ...
    doc.moveTo(signX, currentY).lineTo(signX, currentY + footerH).stroke("#000000");
...
const renderers: Record<
    InvoicePdfTemplate,
    (doc: PDFKit.PDFDocument, data: InvoicePdfData, config: TemplateConfig) => void
> = {
...
export const createInvoicePdfDocument = (data: InvoicePdfData): PDFKit.PDFDocument => {
