...
const hasSessionCookies = (req: Request): boolean => {
  ...
  return Boolean(
    cookies[authCookieNames.access] || cookies[authCookieNames.refresh],
  );
};
  ...
  if (!source) {
    return res.status(403).json({
      ...
      error: {
        ...
        message:
          "Origin or Referer header required for state-changing requests",
      },
    });
  }
...
const productionRateLimit = process.env.NODE_ENV === "production";

export const globalRateLimit = productionRateLimit
  ? createLimiter(env.RATE_LIMIT_WINDOW_MS, env.RATE_LIMIT_MAX_REQUESTS)
  : noopMiddleware;

export const authRateLimit = productionRateLimit
  ? createLimiter(
