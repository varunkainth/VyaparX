...
const invoiceEmailQueue = createQueue<SendInvoiceEmailJobData>(
...
export const enqueueInvoicePdfGeneration = async (data: {
  business_id: string;
  invoice_id: string;
}) => {
  ...
  try {
    return await pdfGenerationQueue.add(
      `invoice:${data.invoice_id}:pdf`,
      data,
      {
        attempts: 3,
        backoff: {
...
export const startWorkers = () => {
  ...
  invoiceEmailWorker.on("failed", (job, error) => {
    console.error(
      `[queue] invoice email job failed (${job?.id ?? "unknown"})`,
      error,
    );
  });
  ...
  const pdfWorker = new Worker<{ business_id: string; invoice_id: string }>(
    pdfGenerationQueueName,
    async (job) =>
      ensureInvoicePdfStored(job.data.business_id, job.data.invoice_id, {
  ...
  pdfWorker.on("failed", (job, error) => {
    console.error(
      `[queue] invoice pdf job failed (${job?.id ?? "unknown"})`,
      error,
    );
  });
