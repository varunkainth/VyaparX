...
import { ensureInvoicePdfStored, getStoredInvoicePdfBuffer } from "./invoice-pdf-storage.service";
...
export const sendInvoiceEmailNow = async ({
    ...
    if (isR2StorageEnabled()) {
        const storedPdf = await ensureInvoicePdfStored(business_id, invoice_id);
        pdfBuffer =
            (await getStoredInvoicePdfBuffer(business_id, invoice_id)) ??
        ...
        if (!storedPdf.pdf_url) {
            throw new AppError("Stored invoice PDF URL missing", 500, ERROR_CODES.INTERNAL_ERROR);
        }
    } else {
