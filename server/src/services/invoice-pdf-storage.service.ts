...
const buildInvoiceDetailForPdf = async (
  businessId: string,
  invoiceId: string,
  ...
  const [
    ...
    invoice.reference_invoice_id
      ? invoiceRepository.getInvoiceSummary(
          businessId,
          String(invoice.reference_invoice_id),
        )
...
export const buildInvoicePdfAsset = async (
  businessId: string,
  invoiceId: string,
) => {
  ...
  const safeInvoiceNo = sanitizeFileSegment(
...
export const ensureInvoicePdfStored = async (
  ...
  const pdfState = await invoiceRepository.getInvoicePdfState(
    businessId,
    invoiceId,
  );
  if (!pdfState) {
  ...
  const invoiceSettings =
    await invoiceSettingsRepository.getOrCreate(businessId);
  ...
  await invoiceRepository.markInvoicePdfProcessing(
    businessId,
    invoiceId,
    expectedTemplate,
  );
  ...
  try {
    ...
    await invoiceRepository.markInvoicePdfFailed(
      businessId,
      invoiceId,
      message,
    );
...
export const getStoredInvoicePdfBuffer = async (
  businessId: string,
  invoiceId: string,
) => {
  const pdfState = await invoiceRepository.getInvoicePdfState(
  ...
  if (!pdfState?.pdf_object_key) return null;

  try {
    return await downloadPdfFromR2(pdfState.pdf_object_key);
  } catch (err: unknown) {
    if ((err as { Code?: string }).Code === "NoSuchKey") {
      console.warn(`[r2] Key not found in bucket: ${pdfState.pdf_object_key}`);
      return null;
    }
    throw err;
  }
...
export const markInvoicePdfPending = async (
  businessId: string,
  invoiceId: string,
) => {
