...
import {
  handleLowStockTransition,
  maybeSendLowStockEmail,
...
const computeInvoiceTotals = (computedItems: ComputedInvoiceItem[]) => {
  const subtotal = round2(
    computedItems.reduce((sum, row) => sum + row.baseAmount, 0),
  );
  const taxableAmount = round2(
    computedItems.reduce((sum, row) => sum + row.taxableValue, 0),
  );
  const cgstAmount = round2(
    computedItems.reduce((sum, row) => sum + row.cgstAmount, 0),
  );
  const sgstAmount = round2(
    computedItems.reduce((sum, row) => sum + row.sgstAmount, 0),
  );
  const igstAmount = round2(
    computedItems.reduce((sum, row) => sum + row.igstAmount, 0),
  );
  const totalTax = round2(cgstAmount + sgstAmount + igstAmount);
  const roundedTotals = calculateRoundOff(
    computedItems.reduce((sum, row) => sum + row.totalAmount, 0),
  );
...
const computeLine = (
  ...
  if (item.quantity <= 0) {
    throw new AppError(
      `Invalid quantity for item ${item.item_name}`,
      400,
...
const noteEffectMap: Record<
  "sales" | "purchase",
  Record<InvoiceNoteType, NoteEffect>
> = {
  sales: {
    credit_note: {
      balanceSign: -1,
...
const formatNoteDescription = (
  noteType: InvoiceNoteType,
  originalInvoiceNumber: string,
) =>
  `${noteType === "credit_note" ? "Credit note" : "Debit note"} for invoice ${originalInvoiceNumber}`;

const isInvoiceNumberDuplicateError = (err: any) =>
  err?.code === "23505" &&
  err?.constraint === "invoices_business_id_invoice_number_key";

async function insertInvoiceWithNumberRetry(params: {
  client: any;
  businessId: string;
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    ...
    try {
      ...
      const invoiceId = await params.insert(invoiceNumber);
      await params.client.query(`RELEASE SAVEPOINT ${savepointName}`);
      return { invoiceId, invoiceNumber, retryCount };
    } catch (err: any) {
      await params.client.query(`ROLLBACK TO SAVEPOINT ${savepointName}`);
      await params.client.query(`RELEASE SAVEPOINT ${savepointName}`);

      if (isInvoiceNumberDuplicateError(err) && attempt < maxRetries) {
        retryCount += 1;
        continue;
      }
  ...
  throw new AppError(
    "Failed to reserve invoice number",
    500,
...
export async function createSaleInvoice(data: CreateInvoiceInput) {
  ...
  const pendingLowStockEmails: Array<
    Parameters<typeof maybeSendLowStockEmail>[0]
  > = [];
  ...
  try {
    ...
    if (
      ...
      (typeof data.round_off === "number" &&
        !moneyClose(data.round_off, roundOff)) ||
    ...
    if (!idempotencyStart.shouldExecute) {
      ...
      return idempotencyStart.cachedResponse as {
        success: boolean;
        invoice_id: string;
      };
    }

    const { invoiceId, invoiceNumber, retryCount } =
      await insertInvoiceWithNumberRetry({
        client,
        businessId: data.business_id,
        financialYear,
        sequenceType: "sales",
    ...
    if (retryCount > 0) {
      console.warn(
        "[InvoiceNumberRetry] Sales invoice number collision recovered",
        {
          business_id: data.business_id,
          invoice_type: "sales",
          retries: retryCount,
    ...
    for (const row of computedItems) {
      ...
      if (row.item.item_id) {
        const stockRow = await invoiceRepository.lockItemStock(
          client,
          row.item.item_id,
        );
        if (!stockRow) {
          throw new AppError(
            `Inventory item not found: ${row.item.item_id}`,
            404,
        ...
        await invoiceRepository.decrementItemStock(
          client,
          row.item.item_id,
          row.item.quantity,
        );
    ...
    const partyRow = await invoiceRepository.lockPartyBalance(
      client,
      data.party_id,
    );
    if (!partyRow) {
    ...
    await invoiceRepository.updatePartyBalance(
      client,
      data.party_id,
      newBalance,
    );
    ...
    await Promise.all(
      pendingLowStockEmails.map((context) =>
        maybeSendLowStockEmail(context, true),
      ),
    );
    ...
    void trackAnalyticsEvent({
      ...
      event_data: {
        ...
        invoice_type: "sales",
        uses_inclusive_price: computedItems.some(
          (item) => item.price_mode === "inclusive",
        ),
        invoice_number_retry_count: retryCount,
      },
    });
...
export async function createPurchaseInvoice(data: CreateInvoiceInput) {
  ...
  try {
    ...
    if (retryCount > 0) {
      console.warn(
        "[InvoiceNumberRetry] Purchase invoice number collision recovered",
        {
          business_id: data.business_id,
          invoice_type: "purchase",
          retries: retryCount,
    ...
    for (const row of computedItems) {
      ...
      if (row.item.item_id) {
        ...
        const currentStock = Number(stockRow.current_stock);
        const nextStock = round2(currentStock + row.item.quantity);
        await invoiceRepository.incrementItemStock(
          client,
          row.item.item_id,
          row.item.quantity,
        );
...
export async function createInvoiceNote(data: CreateInvoiceNoteInput) {
  ...
  try {
    ...
    if (!Array.isArray(data.items) || data.items.length === 0) {
      throw new AppError(
        "Invoice notes must contain at least one item",
        400,
        ERROR_CODES.BAD_REQUEST,
      );
    }
    ...
    if (
      referenceInvoice.invoice_type !== "sales" &&
      referenceInvoice.invoice_type !== "purchase"
    ) {
      throw new AppError(
        "Reference invoice cannot have credit/debit notes",
        400,
        ERROR_CODES.BAD_REQUEST,
      );
    }
    ...
    const sequenceType =
      data.note_type === "credit_note" ? "sales" : "purchase";
    ...
    if (retryCount > 0) {
      console.warn(
        "[InvoiceNumberRetry] Invoice note number collision recovered",
        {
          business_id: data.business_id,
    ...
    const effect =
      noteEffectMap[referenceInvoice.invoice_type as "sales" | "purchase"][
        data.note_type
      ];
...
export async function listInvoices(input: ListInvoicesInput) {
  ...
  if (input.search) {
    ...
    where.push(
      `(i.invoice_number ILIKE $${values.length} OR p.name ILIKE $${values.length})`,
    );
  }
...
export async function getInvoiceById(businessId: string, invoiceId: string) {
  ...
  const [
    ...
    invoice.reference_invoice_id
      ? invoiceRepository.getInvoiceSummary(
          businessId,
          invoice.reference_invoice_id,
        )
...
export async function cancelInvoice(args: CancelInvoiceInput) {
  ...
  try {
    ...
    const invoice = await invoiceRepository.lockInvoiceForCancel(
      client,
      args.business_id,
      args.invoice_id,
    );
    ...
    if (invoice.is_cancelled) {
      throw new AppError(
        ...
        409,
        ERROR_CODES.INVOICE_ALREADY_CANCELLED,
      );
    }
    ...
    if (invoice.invoice_type === "sales") {
      const items = await invoiceRepository.getInvoiceItemsForCancel(
        client,
        args.invoice_id,
      );
      ...
      for (const row of items) {
        ...
        const locked = await invoiceRepository.lockItemStock(
          client,
          row.item_id,
        );
        if (!locked) {
          throw new AppError(
            `Inventory item not found: ${row.item_id}`,
            404,
            ERROR_CODES.NOT_FOUND,
          );
        }
    ...
    await invoiceRepository.markCancelled(
      client,
      args.cancelled_by,
      args.cancel_reason ?? null,
      args.invoice_id,
    );
